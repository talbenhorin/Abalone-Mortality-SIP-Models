...
sip <- function(time, state, parameters) {
  ...
  with(as.list(c(state, parameters)), {
    ...
    dSg <- b*(Sg+Ig) - beta*Sg*P - (mu+c*(Sg+Ig))*Sg
    dIg <- beta*Sg*P - (mu+rg+c*(Sg+Ig))*Ig
    dP <-  z*(I+Ig) - gamma*P
    
    return(list(c(dS, dI, dSg, dIg, dP)))
  })
}

### Set parameters
init       <- c(S = 5, I = 2, Sg = 5, Ig = 2, P = 0)
## b: reproduction; beta: transmission; mu: natural mortality; r: WS mortality; c: parasite shedding; gamma: parasite loss
parameters <- c(b = 0.32, beta = 0.03, mu = 0.15, r = 0.8, c = 0.025, z = 200, gamma = 52, rg = 0.05)
## Time frame
